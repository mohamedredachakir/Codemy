...
<div class="max-w-4xl mx-auto">
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-black text-slate-900 tracking-tight">Modifier le <span class="text-indigo-600">Sprint</span></h1>
        <p class="text-slate-500 font-medium">Mise à jour du cycle : <span class="text-slate-800 font-bold">{{ $sprint->name }}</span></p>
    </div>

    <div class="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 p-8 md:p-12">
        <form action="{{ route('sprints.update', $sprint->id) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="md:col-span-3">
                    ...
                    <input type="text" name="name" value="{{ old('name', $sprint->name) }}" required
                ...
                <div>
                    ...
                    <input type="number" name="duration" value="{{ old('duration', $sprint->duration) }}" required
                ...
                <div>
                    ...
                    <input type="number" name="order" value="{{ old('order', $sprint->order) }}" required
                        class="w-full px-5 py-4 rounded-2xl border border-slate-200 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all">
                </div>
            </div>
            ...
            <div class="pt-6 border-t border-slate-100">
                ...
                <div class="relative">
                    <select name="competences[]" multiple
                        class="w-full px-5 py-4 rounded-2xl border border-slate-200 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all min-h-[200px] bg-slate-50/30 shadow-inner">
                        @foreach($competences as $competence)
                            <option value="{{ $competence->id }}"
                                {{ $sprint->competences->contains($competence->id) ? 'selected' : '' }}
                                class="py-2 px-2 rounded-lg mb-1 font-medium transition-colors">
                                {{ $competence->label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <p class="mt-3 text-xs text-slate-400 font-medium italic">
                    Astuce : Maintenez Ctrl (Win) ou Cmd (Mac) pour modifier la sélection multiple.
                </p>
            </div>

            <div class="pt-6 flex flex-col md:flex-row gap-4">
                <button type="submit"
                    class="flex-1 bg-slate-900 text-white py-4 rounded-2xl font-bold hover:bg-indigo-600 transition-all duration-300 shadow-lg shadow-indigo-100 flex items-center justify-center transform active:scale-95">
