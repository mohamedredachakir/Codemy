...
<div class="max-w-2xl mx-auto">
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-black text-slate-900 tracking-tight">Ajouter une <span class="text-indigo-600">Compétence</span></h1>
        <p class="text-slate-500 font-medium">Définissez un nouveau critère pour l'évaluation des étudiants.</p>
    </div>

    <div class="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 p-8 md:p-10">
        <form action="{{ route('competences.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" name="code" value="{{ old('code') }}" required
                        class="w-full pl-11 pr-5 py-4 rounded-2xl border @error('code') border-red-300 @else border-slate-200 @enderror focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all placeholder:text-slate-400"
                        placeholder="Ex: 101">
                </div>
                @error('code')
            ...
            <div>
                <label class="block text-sm font-bold text-slate-700 mb-2 ml-1">Libellé (Description courte)</label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    ...
                    <input type="text" name="label" value="{{ old('label') }}" required
                        class="w-full pl-11 pr-5 py-4 rounded-2xl border @error('label') border-red-300 @else border-slate-200 @enderror focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all placeholder:text-slate-400"
                        placeholder="Ex: Maquetter une application">
                </div>
                @error('label')
                    <p class="mt-2 text-xs text-red-600 font-bold flex items-center">
                        <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path></svg>
